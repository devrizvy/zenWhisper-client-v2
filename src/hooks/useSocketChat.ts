...
import socketService, { type PrivateMessage, type RoomMessage } from '../services/socketService';
...
export const useSocketChat = ({ userEmail, username, autoConnect = true }: UseSocketChatOptions) => {
  ...
  useEffect(() => {
    ...
    const handleUserStatusChange = (data: { userEmail: string; status: string }) => {
      setOnlineUsers(prev => {
    ...
    const handleUserTyping = (data: { userEmail: string; username: string; isTyping: boolean }) => {
      setTypingUsers(prev => ({
        ...prev,
    ...
    const handleJoiningMessage = (message: { author: string; message: string; time: string }) => {
      // This could be displayed as a system message in the current room
      console.log('User joined:', message);
    };

    const handleLeaveMessage = (message: { author: string; message: string; time: string }) => {
      // This could be displayed as a system message in the current room
  ...
  const joinPrivateChat = useCallback((chatId: string) => {
  ...
  const getUserTypingStatus = useCallback((userEmail: string) => {
