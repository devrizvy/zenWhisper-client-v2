...
class SocketService {
  ...
  onNewMessageNotification(callback: (data: { from: string; chatId: string; message: string }) => void): void {
    this.socket?.on('new_message_notification', callback);
  }

  onUserStatusChange(callback: (data: { userEmail: string; status: string }) => void): void {
    this.socket?.on('user_status_change', callback);
  }

  // Typing indicators for private chat
  startTyping(data: { chatId: string; userEmail: string; username: string }): void {
    this.socket?.emit('typing_start', data);
  }

  stopTyping(data: { chatId: string; userEmail: string; username: string }): void {
    this.socket?.emit('typing_stop', data);
  }

  onUserTyping(callback: (data: { userEmail: string; username: string; isTyping: boolean }) => void): void {
  ...
  onJoiningMessage(callback: (message: { author: string; message: string; time: string }) => void): void {
    this.socket?.on('joining_message', callback);
  }

  onLeaveMessage(callback: (message: { author: string; message: string; time: string }) => void): void {
